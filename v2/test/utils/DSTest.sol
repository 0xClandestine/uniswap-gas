...
contract DSTest {
        ...
        if (hasHEVMContext()) {
            (bool status,) = HEVM_ADDRESS.call(
                abi.encodePacked(
                    ...
                    abi.encode(
                        HEVM_ADDRESS, bytes32("failed"), bytes32(uint256(0x01))
                    )
                )
            );
    ...
    function hasHEVMContext() internal view returns (bool) {
        ...
        assembly {
            hevmCodeSize :=
                extcodesize(0x7109709ECfa91a80626fF3989D68f67F5b1DD12D)
        }
    ...
    function assertNotEqDecimal(int256 a, int256 b, uint256 decimals)
        internal
    {
        if (a == b) {
            emit log("Error: a != b not satisfied [decimal int]");
    ...
    function assertNotEqDecimal(uint256 a, uint256 b, uint256 decimals)
        internal
    {
        if (a == b) {
            emit log("Error: a != b not satisfied [decimal uint]");
    ...
    function assertNotEq(string memory a, string memory b, string memory err)
        internal
    {
        if (keccak256(abi.encodePacked(a)) == keccak256(abi.encodePacked(b))) {
            emit log_named_string("Error", err);
    ...
    function assertNotEq0(bytes memory a, bytes memory b, string memory err)
        internal
    {
        if (checkEq0(a, b)) {
            emit log_named_string("Error", err);
